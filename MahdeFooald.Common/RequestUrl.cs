...
namespace MahdeFooald.Common
{
    ...
    //new_date_deadline_Task
    {
        private const string baseurl = "http://80.210.26.4:8585//MFKian/api/data/v9.0/";
        private const string taskurl = "http://80.210.26.4:8585//MFkian/main.aspx?etn=";
        private const string _taskSelectedItem = @"activityid,new_remained_time_hour,new_remaining_days,new_task_status,new_task_type,subject,prioritycode,description,_ownerid_value,new_notification_threshold";
        private const string _settingCrmSelectedItem = @"new_api_url,new_baseurl,new_day_filter,new_domain_name,new_entity_name, new_expired_task_code,new_log_path,new_name,new_notification_message,new_notification_systemid,new_setting_timer,new_task_api,new_task_status, new_task_types,new_time_awaited,new_time_check";
        ...
        public static string BuildUrl(UrlBuilderMode mode, string ownerId, string tasksId,string userId)
        {
            if (mode == UrlBuilderMode.SingleTask && string.IsNullOrEmpty(tasksId))
                ...
                throw new ArgumentNullException("userid or taskid sent null to the method ");
            else if(mode == UrlBuilderMode.UpdateTask && string.IsNullOrEmpty(tasksId))
                throw new ArgumentNullException("userid or taskid sent null to the method ");
            ...
            return mode switch
            {
                ...
                UrlBuilderMode.UpdateTask => $"{baseurl}tasks({tasksId})",
